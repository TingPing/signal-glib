...
#define KEY_GROUP "signed-pre-keys"

#include "signed-pre-key-store.h"
#include "key-utils.h"

static int
load_signed_pre_key (axolotl_buffer **record,
					 ...
					 void *user_data)
{
  ...
  g_snprintf (key, sizeof(key), "%"G_GUINT32_FORMAT, signed_pre_key_id);
...
static int
store_signed_pre_key (uint32_t signed_pre_key_id,
				      uint8_t *record,
				      size_t record_len,
				      void *user_data)
{
...
sg_signed_pre_key_store_new (GKeyFile *keystore)
{
  axolotl_signed_pre_key_store *store = g_new (axolotl_signed_pre_key_store, 1);
